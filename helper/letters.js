var Letters = function (game, word, scale, startPoss) {
	...
	this.poss = copy(startPoss);
	this.scale = scale;
	this.word = null;
	this.letterGab = 3;
	this.lettersList = null;

	this.rightSideOfScreen = false;
	this.leftSideOfScreen = false;
	this.topOfScreen = false;
	this.bottomOfScreen = false;
	this.center = function(width, height){
		if(width != null){
			this.poss.x = this.startPoss.x + ((width - this.lettersList.width) / 2);
		}
		if(height != null){
			this.poss.y = this.startPoss.y + ((height - this.lettersList.height) / 2);
		}
	}
	this.RightSideOfScreen = function () {
		this.rightSideOfScreen = true;
		this.poss.x = this.game.can.width - this.lettersList.width - this.startPoss.x;
	}
	this.LeftSideOfScreen = function () {
		this.leftSideOfScreen = true;
		this.poss.x = 0 + this.startPoss.x;
	}
	this.TopOfScreen = function () {
		this.topOfScreen = true;
		this.poss.y = 0 + this.startPoss.y;
	}
	this.BottomOfScreen = function () {
		this.bottomOfScreen = true;
		this.poss.y = this.game.can.height - this.lettersList.height - this.startPoss.y;
	}
	this.render = function(){
		var currentWidth = 0;
		for (var i = 0; i < this.lettersList.letters.length; i++) {
			var image = this.lettersList.letters[i];				
			this.game.ctx.drawImage(image, 0, 0, image.width, image.height, (this.poss.x + currentWidth), this.poss.y, image.width / this.scale, image.height / this.scale);
			currentWidth += (image.width + this.letterGab) / this.scale;
		}
	}
	this.setText = function(word){
		...
		for (var i = 0; i < word.length; i++) {
			...
			if(this.lettersList.height < (letterImage.height / this.scale)){
		...
		if (this.rightSideOfScreen) {
			this.RightSideOfScreen();
		}
		if (this.leftSideOfScreen) {
			this.ReftSideOfScreen();
		}
		if (this.topOfScreen) {
			this.TopOfScreen();
		}
		if (this.bottomOfScreen) {
			this.BottomOfScreen();
		}
	}
	this.setText(word);
